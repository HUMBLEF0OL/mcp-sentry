...
function runScan(target: string, outFile: string): Finding[] {
	const r = spawnSync(
		process.execPath,
		[cliEntry, 'scan', target, '--format', 'json', '--output', outFile],
		{
			stdio: 'inherit',
		},
	);
...
async function main(): Promise<void> {
	...
	if (entries.length === 0) {
		process.stderr.write(
			'measure-fp: no usable corpus entries (all TODO_SHA?). Pin SHAs in corpus.txt.\n',
		);
		process.exit(2);
	}
	...
	for (const entry of entries) {
		...
		for (const f of findings) {
			...
			if (rl) {
				...
				const ans = (await prompt(rl, '[y]es=true positive, [n]o=false positive, [s]kip: '))
					.trim()
					.toLowerCase();
				if (ans === 'n') {
					falsePositive++;
