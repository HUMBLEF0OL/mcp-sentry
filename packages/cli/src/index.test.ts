...
describe('CLI flag wiring', () => {
	...
	it('rejects --format sarif until Phase 3 lands', async () => {
		const exitSpy = vi.spyOn(process, 'exit').mockImplementation(((code?: number) => {
			throw new Error(`__exit:${code ?? 0}`);
		}) as never);
		const errSpy = vi.spyOn(process.stderr, 'write').mockImplementation(() => true);
		try {
			await expect(
				main(['node', 'mcp-sentry', '--format', 'sarif', 'scan', '.']),
			).rejects.toThrow(/__exit:1/);
			const stderr = errSpy.mock.calls.map((c) => String(c[0])).join('');
			expect(stderr).toMatch(/sarif/i);
		} finally {
			exitSpy.mockRestore();
			errSpy.mockRestore();
		}
...
describe('--report deferred-notice (G3)', () => {
	it('emits a stderr notice and proceeds when --report is passed', async () => {
		const fixture = path.resolve(here, '..', 'fixtures', 'clean-server');
		const errSpy = vi.spyOn(process.stderr, 'write').mockImplementation(() => true);
		const outSpy = vi.spyOn(process.stdout, 'write').mockImplementation(() => true);
		try {
			...
			const stderr = errSpy.mock.calls.map((c) => String(c[0])).join('');
			expect(stderr).toMatch(/--report is not yet wired/);
		} finally {
			errSpy.mockRestore();
			outSpy.mockRestore();
		}
